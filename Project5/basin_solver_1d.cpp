...
basin_solver_1d::basin_solver_1d(double dx, int N_x,
...
void basin_solver_1d::set_initial_condition(double* init_psi, double* init_zeta){
    ...
    if (fabs(init_psi[0] - this->bc_0) > eps || fabs(init_psi[this->N_x-1] - this->bc_N) > eps){
...
void basin_solver_1d::basin_euler(){
    ...
    alloc_array_1D(psi_prev, this->N_x);
    alloc_array_1D(psi_curr, this->N_x);
    alloc_array_1D(zeta_prev, this->N_x);
    alloc_array_1D(zeta_curr, this->N_x);

    double *diag, *rhs_tridiag;                 // Arrays needed for call to tridiag solver
    alloc_array_1D(diag, this->N_x-2);
    alloc_array_1D(rhs_tridiag, this->N_x-2);
    ...
    psi_curr[0] = this->bc_0; psi_curr[this->N_x-1] = this->bc_N;                 // Also apply BC here
    zeta_curr[0] = zeta_prev[0]; zeta_curr[this->N_x-1] = zeta_prev[this->N_x-1];   // Also apply BC here
    ...
    for (int n = 1; t < T; n++){
        ...
        for (int i = 1; i < this->N_x-1; i++){    // Loop over all interior zeta-values
            rhs_tridiag[i-1] = -dxdx*zeta_curr[i];     // Right-hand-side of Poisson eq.
        }

        tridiag_ferrari(diag, rhs_tridiag, this->N_x-2, psi_curr+1);   // Solve Poisson eq. in interior
...
void basin_solver_1d::basin_leapfrog(){
    ...
    alloc_array_1D(psi_curr, this->N_x);
    alloc_array_1D(zeta_prev, this->N_x);
    alloc_array_1D(zeta_pp, this->N_x);
    alloc_array_1D(zeta_curr, this->N_x);
    ...
    psi_curr[0] = this->bc_0; psi_curr[this->N_x-1] = this->bc_N;             // Also apply BC for curr arrays
    zeta_curr[0] = zeta_pp[0]; zeta_curr[this->N_x-1] = zeta_pp[this->N_x-1];   // Also apply BC for curr arrays
    ...
    for (int i = 1; i < this->N_x-1; i++){    // Loop over all interior zeta-values
        rhs_tridiag[i-1] = -dxdx*zeta_prev[i];     // Right-hand-side of Poisson eq.
    }

    tridiag_ferrari(diag, rhs_tridiag, this->N_x-2, psi_prev+1);   // Solve Poisson eq. in interior
