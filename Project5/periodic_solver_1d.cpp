...
periodic_solver_1d::periodic_solver_1d(double dx, int N,
...
void periodic_solver_1d::set_initial_condition(double* init_psi, double *init_zeta){
    ...
    if ((fabs(init_psi[0] - init_psi[this->N-1]) > eps) || (fabs(init_zeta[0] - init_zeta[this->N-1]) > eps)){
...
void periodic_solver_1d::periodic_euler(){
    ...
    arma::vec psi_prev(this->N);
    arma::vec psi_curr(this->N);
    arma::vec zeta_prev(this->N);
    arma::vec zeta_curr(this->N);

    arma::vec rhs_poisson(this->N);     // Vector to hold right-hand-side of Poisson eq.
    arma::mat A = arma::zeros(this->N, this->N);    // Matrix to store second-derivative coefficients

    /* Initialize periodic BC matrix to pass to Armadillo to solve Poisson at every time step. */
    for (int i = 0; i < this->N; i++){
    ...
    A(0,this->N-2) = -1.0;      // Semi-corners value due to periodic BC
    A(this->N-1,1) = -1.0;      // Semi-corners value due to periodic BC
...
//        zeta_curr[0] = zeta_prev[0] - alpha*(psi_prev[1] - psi_prev[this->N-1]);                    // Periodic BC
//        zeta_curr[this->N-1] = zeta_prev[this->N-1] - alpha*(psi_prev[0] - psi_prev[this->N-2]);    // Periodic BC

        zeta_curr[0] = zeta_prev[0] - alpha*(psi_prev[1] - psi_prev[this->N-2]);                    // Periodic BC
        zeta_curr[this->N-1] = zeta_prev[this->N-1] - alpha*(psi_prev[1] - psi_prev[this->N-2]);    // Periodic BC

        /* STEP 2: Solve the 1D Poisson equation to update streamfunction. */
        for (int i = 0; i < this->N; i++){    // Loop over all interior zeta-values
...
void periodic_solver_1d::periodic_leapfrog(){
    ...
    zeta_prev[0] = this->zeta_0[0] - alpha*(this->psi_0[1] - this->psi_0[this->N-2]);                    // Periodic BC
    zeta_prev[this->N-1] = this->zeta_0[this->N-1] - alpha*(this->psi_0[1] - this->psi_0[this->N-2]);    // Periodic BC
    ...
    for (int n = 2; t < T; n++){
        ...
        zeta_curr[0] = zeta_pp[0] - gamma*(psi_prev[1] - psi_prev[this->N-2]);                    // Periodic BC
        zeta_curr[this->N-1] = zeta_pp[this->N-1] - gamma*(psi_prev[1] - psi_prev[this->N-2]);    // Periodic BC
