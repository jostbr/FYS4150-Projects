...
void nbody_solver::verlet_GR(double h, double t_max, int frame_write){
    ...
    for (int i = 0; i < this->num_bodies; i++){
        ...
        for (int dim = 0; dim < cnst::num_dims; dim++){
            bodies_curr[i].a[dim] = this->compute_total_acc_GR(this->bodies[i], this->bodies, dim);
        }
    }
    ...
    while (t <= t_max){
        ...
        /* New loop over bodies and directions to compute the updated values for acceleration
            ...
            for (int i = 0; i < this->num_bodies; i++){
                //std::cout << "Time stepping for " << this->bodies[i].name << std::endl;
                for (int dim = 0; dim < cnst::num_dims; dim++){
                    this->bodies[i].a[dim] = this->compute_total_acc_GR(this->bodies[i], this->bodies, dim);
                    this->bodies[i].v[dim] = bodies_curr[i].v[dim] + h_half*
                            (this->bodies[i].a[dim] + bodies_curr[i].a[dim]);
                }
    ...
    for (int i =0; i < this->num_bodies; i++){
        ...
        std::cout << theta_p << "  " << x_p[i] << "  " << y_p[i] << std::endl;
    }
    delete[] bodies_curr;
}
...
double nbody_solver::compute_total_acc(planet subject, planet* objects, int dim) const {
    ...
    if (this->fixed_sun == true){
        ...
        if (subject.name.compare("mercury_GR") == 0){
            total_accel += subject.compute_acc_mercury_GR(sun, dim);
        }
        else {total_accel += subject.compute_acc(sun, dim);}     // Include force from sun separately
    }
...
double nbody_solver::compute_total_acc_GR(planet subject, planet* objects, int dim) {
    double total_accel = 0.0;
